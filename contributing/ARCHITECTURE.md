...
- Keys dedup on the **UUID being resolved** (`allocate(dedupKey, baseName, suffixHint)`'s `dedupKey`), not the derived name, so repeat references to the *same* UUID still collapse onto one variable.
- When two *different* UUIDs would otherwise produce the same base name, disambiguates the second (and subsequent) name(s) using a caller-supplied `suffixHint` — **not the UUID itself**. The UUID is per-environment API data; baking it into the variable *name* would make the same logical reference site produce a different variable name every time the same configuration is exported from a different environment, breaking the "one checked-in module, per-environment tfvars" model. Instead, each caller supplies a stable, environment-portable identifier for the *referencing* side:
  - `orchestrator.go` passes the referencing resource's own Terraform label (`rd.Label`) — e.g. two populations referencing different theme UUIDs disambiguate to `pingone_branding_theme_id_pingcli__sam_population`, not a UUID fragment.
  - `embedded_references.go` passes the accumulated wildcard-traversal key (e.g. a DaVinci node's own `data.id`, captured as `walkAndProcessPath` descends `nodes.*`) — stable across environments for the same flow, unlike the target UUID.
- On the rare case that the hint itself also collides (two different dedupKeys sharing both the base name and the hint), appends a numeric tiebreaker (`_2`, `_3`, ...) rather than silently reusing a claimed name.

Any new fallback-variable-producing code path must route its generated name through this shared allocator, passing a stable identifier from the *referencing* side as the hint — never the target UUID — rather than deduping on the name string directly. Deduping on the name alone (with no per-instance identifier at all) is the root cause of #130 and #138 (silent misattribution when two distinct target instances share a derived name); baking the UUID into the hint would trade that bug for environment-portability breakage instead.
