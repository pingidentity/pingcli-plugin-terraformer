	...
	for _, exportedData := range results {
		for _, rule := range rules {
			...
			for _, resource := range exportedData.Resources {
				processResourceWithRule(resource, rule, g)
			}
		}
	}
}
...
func processResourceWithRule(resource *ResourceData, rule EmbeddedReferenceRule, g *graph.DependencyGraph) {
	...
	walkAndProcessPath(resource.Attributes, pathSegments, 0, rule, resource, g)
}
			...
			for key := range typedCurrent {
				...
				if isLastSegment {
					...
					if rawValue, ok := nextValue.(RawHCLValue); ok {
						processedValue := processRawHCLValue(rawValue, rule, resource, g)
						typedCurrent[key] = processedValue
					}
				} else {
					// Continue walking deeper
					walkAndProcessPath(nextValue, pathSegments, segmentIndex+1, rule, resource, g)
				}
			}
		} else {
			// Regular key lookup
...
// and replaces it with a Terraform reference
func processRawHCLValue(
	...
	// Look up the UUID in the graph
	refName, err := g.GetReferenceName(rule.TargetResourceType, uuid)
	if err != nil {
		// UUID not found in graph - leave unchanged
		return value
	}
